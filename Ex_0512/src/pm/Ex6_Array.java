...
//		a1과 a2라는 1차원배열을 한곳에 모아두는 2차원 배열을 만들자!
		String[][] ar = new String[2][4];
		
//		준비된 2차원배열 각 요소에 a1괴 a2를 저장하자!
		ar[0] = a1;
		ar[1] = a2;
...
//		다음과 같이 하고자 한다면 선언, 생성, 초기화를 한번에 하면 된다.
		...
		ar3[0] = ar; // 3차원 배열의 첫번째 요소에 2차원배열 ar을 저장
		ar3[1] = ar2; 
					
		for(int i = 0; i < ar3.length; i ++) { // 3차원 배열에 저장된 2차원 배열을 반복할 for
			System.out.println("2차원배열" + (i+1));
			
			for(int j = 0; j < ar3[i].length; j ++) { // 2차원 배열에 저장된 1차원배열 반복문
				System.out.print("\r\n\t"); // 줄바꿈 
				
				for(int k = 0; k <ar3[i][j].length; k++) { // [][] 잊지 말자 1차원 배열에 저장된 값들 반복처리 
