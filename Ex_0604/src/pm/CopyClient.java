			...
			try {
				...
				if(obj != null) {
					...
					case 2:
						...
						break;
					case 4 : // 방만들기 
						currentRoom = new ChatRoom(protocol.getMsg(), server); // 방제목과 서버의 주소를 가져옴
						server.r_list.add(currentRoom); // 방리스트에 방 저장 
						
						// 서버에 있는 u_list(사용자대기실)에서 복사본(CopyClient)을 지워야함
